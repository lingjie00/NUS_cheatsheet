...
Form $p_0 := x + a_0$ s.t. $\int w(x) p_0 dx = 0$ and $p_1 := x^2 + b_1 x + b_0$ s.t. $\int w(x) p_1 dx = \int w(x) x p_1 dx = 0$

$p_m = x p_{m-1} + a_m p_{m-1} + b_m p_{m-2}$ s.t. $\int w(x) x^m p_m dx = \int w(x) x^{m-1}p_m dx = 0$
Require \circled{1} solve $x_0, \cdots, x_m$ as solutions to $p_m = 0$ (roots)
\circled{2} find $c_0, \cdots, c_m$ as solutions to $\int r(x) dx = \sum c_j r(x_j)$ where $r$ is polynomial of order $m-1$
...
\subsection{BS unbiased estimate}
$$
...
$$
    R(X_t, Y_t) = \frac{f(Y_t)g(X_t)}{f(X_t)g(Y_t)}
$$

require $g_Y(X_t|Y_t) = g_Y(X_t)$ and $g_Y(Y_t|X_t) = g_Y(Y_t)$.
Note $g$ is the density of $Y$ the proposal distribution.
