...
\compact
\subsection{Real Convergence}
$\lim_{n\rightarrow\infty}(1+\frac{x}{n})^n = \exp(x)$
\compact
\subsection{Linear Algebra}
\circled{a} $\left[[a, b], [c, d]\right]^{-1}=\frac{1}{det(A)}\left[[d, -b], [-c, a]\right]^T$,
\circled{b} $\frac{d}{dX}X^T A X = 2AX$, $\frac{d}{dX}A^T XA = AA^T$

\circled{c} $(x-\mu)^T\Sigma^{-1}(x-\mu) = x^T\Sigma^{-1}x - \mu^T \Sigma^{-1}x - x^T\Sigma^{-1}\mu + \mu^T \Sigma^{-1}\mu$
\circled{d} $(A+B)^T = A^T + B^T$
\compact
...
$P(X_{(n)} \leq x) = P(X_1 \leq x)^n$ and $P(X_{(n)} = x)= n P(X_1 \leq x)^{n-1}P(X_1 = x)$,
\compact
$P(X_{(1)} \leq x) = 1 - \left[1 - P(X_1 \leq x)\right]^n$ and
$P(X_{(1)} = x)= n \left[1 - P(X_1 \leq x)\right]^{n-1}P(X_1 = x)$
\compact
\subsection{Moments}
...
$Var(a^TX)=a^TVar(X)a$,
$Var(X) = E(X^2) - E^2(X)$,
$Var(aX + bY) = a^bVar(X) + b^2Var(Y) + 2abCov(X, Y)$
\compact
\subsection{CHF}
...
$E(\hat\mu - \mu)$
\compact
\subsection{Symmetric distribution}
$Y =^D -Y$, $E_{-Y}(Y) = E_Y(-Y)$, mean = mediam = mode
\compact
\subsection{Radon-Nikodym}
$\lambda << \nu$, there exist unique $f$ s.t.
$\lambda(A) = \int_A f d\nu, A\in \mathcal{F}$
and $f(x, \theta) = \frac{d\lambda}{d\nu}$
\compact
\subsection{Gamma family}
$E(0, \theta) = \Gamma(1, \theta)$,
$\Gamma(\frac{n}{2}, 2) \sim \chi_n^2$,
$X\sim U(0, 1) \Rightarrow -\log X \sim E(0, 1)$,
$Y\sim\Gamma(n, \theta_0)$, $\frac{2Y}{\theta_0}\sim\Gamma(n, 2)\sim\chi^2_{2n}$
\compact
\subsection{Common result}
\circled{a} $X\sim\Gamma(\alpha, \gamma)$, $E(X) = \alpha\gamma$, $Var(X) = \alpha\gamma^2$
\circled{b} $Z\sim N(0, 1)$, $W\sim\chi_p^2$, $\frac{Z}{\sqrt{W}/p}\sim t_{p}$
\circled{c} $t^2 \sim \chi^2_a/\chi^2_b \sim F_{a, b}$
\compact
\section{Integration Convergence}
\compact
\subsection{MCT}
...
\compact
\circled{1} $L^p\Rightarrow L^q\Rightarrow P$, $p>q$
\circled{2} $a.s. \Rightarrow P \Rightarrow D$
\circled{3} $X_n\xrightarrow{D} C \Rightarrow X_n \xrightarrow{P} C$
\circled{4} If $X_n\xrightarrow{P} X \Rightarrow \exists$ sub-seq s.t. $X_{n_j}\xrightarrow{\text{a.s.}}X$.
\compact
...
or $\sqrt{n}(\bar{X}-E(X_1))\xrightarrow{D}N(0, \Sigma)$
\compact
...
= - \frac{\partial^2\log\ell(\eta)}{\partial\eta\partial\eta^T}
$
\compact
...
when $\theta$ is fixed and $\varphi$ is unknown, use $E_{\theta}(\cdot|U)$ to denote expectation wrt $f_{\theta, \varphi}$
as $U$ is sufficient for $\varphi$.
$f_{\theta, \varphi}(x) = \exp\left\{\theta Y(x) + \varphi^T U(x) - \mathcal{C}(\theta, \varphi)\right\}$,
\compact
\subsection{UMPU NEF $H_0: \theta\leq \theta_1$ or $\theta\geq \theta_2$ $H_1: \theta_1<\theta<\theta_2$}
$T(Y, U) = I(c_1(U) < Y < c_2(U))$
s.t. $E_{\theta_1}[T|U=u] = E_{\theta_2}[T|U=u] =\alpha$
\compact
...
$T(Y, U) = I(Y < c_1(U)$ or $ Y > c_2(U))$
s.t. $E_{\theta_0}[T|U=u]=\alpha$ and
$E_{\theta_0}[TY| U=u]=\alpha E_{\theta_0}(Y|U=u)$
\compact
\subsection{UMPU NEF $H_0: \theta\leq \theta_0$ $H_1: \theta>\theta_0$}
$T(Y, U) = I(Y > c(U))$
s.t. $E_{\theta_0}[T|U=u]=\alpha$
\compact
...
s.t. $E_{\theta_1}[T] = E_{\theta_2}[T] = \alpha$
\compact
...
\circled{4} $V(y, u)=a(u)y + b(u)$ and $a(u) > 0$
$\Rightarrow T(V) = I(V < c_1$ or $ V > c_2)$
s.t. $E_{\theta_0}[T] = \alpha$
and $E_{\theta_0}[TV] = \alpha E_{\theta_0}(V)$
\compact
...
\compact
\subsection{UMPU results - one sample normal}
\circled{a} $H_0: \sigma^2 = \sigma_0^2$, $H_1: \sigma^2 \neq \sigma_0^2$
$\theta = -(2\sigma^2)^{-1}$, $\varphi = n\mu/\sigma^2$, $Y=\sum X_i^2$, $U\bar{X}$
$V = (n-1)S^2 = Y - nU^2$, $S = \frac{1}{n-1}\sum (X_i - \bar{X})$,
$V/\sigma^2 \sim\chi^2_{n-1}$
\circled{b} $H_0: \mu \leq \mu_0$, $H_1: \mu > \mu_0$,
$Y = \bar{X}$, $U=\sum (X_i - \mu_0)^2$, $\theta=-n(\mu-\mu_0)/\sigma^2$,
$\varphi = -(2\sigma^2)^{-1}$,
$V = (\bar{X} - \mu_0)/\sqrt{U}$ increasing in $Y$, under $H_0$, $V \perp U$,
$t(X) = \sqrt{n}(\bar{X} - \mu_0)/S = \sqrt{(n-1)n}V(X)/\sqrt{1-n[V(X)]^2}\sim t_{n-1}$,
$t(X)$ increasing in $V$
\circled{c} $H_0: \mu=\mu_0$, $H_1:\mu\neq\mu_0$, $V=(\bar{X} - \mu_0)/\sqrt{U}$ symmetric about 0 under $H_0: \mu=\mu_0$,
reject $H_0$ when $|V|>d$ where $P_{H_0}(|V|>d)=\alpha$. consider same $t$ as before.
\compact
\section{Statistics}
\compact
...
\compact
\subsection{Generalised NP - working example}
$H_0: \lambda = 1, \lambda = 2$, $H_1: \lambda \in (1, 2)$
\circled{1} $\max \int \varphi(x) f_{\lambda}(x) dx$ with $\int \varphi f_{\lambda=1} dx = \int \varphi f_{\lambda=2} dx = \alpha$
\circled{2} by generalised NP lemma, $\varphi^*(x) = I(f_\lambda > k_1 f_{\lambda =1} + k_2 f_{\lambda = 2}) = I(c_1 g(x) + c_2 g(x) < 1)$
\circled{3} show $c_i$ are positives. If $c_i$ are both negative then test always reject $H_0$.
If $c_i$ have opposite signs, or one of them equals zero, LHS of inequalities is monotone function of $x$ and test will be one-sided test.
Power will be monotone and unable to satisfy constraints for power function. So $c_i$ must be positive.
\circled{4} Then $c_1f_{\lambda =1 } + c_2 f_{\lambda=2}$ is convex, and $\varphi^*$ is two-sided test with form $\varphi^*(x) = I_{b_1, b_2}(x)$
\circled{5} Find $b_1, b_2$ s.t. $\int \varphi^* f_{\lambda=1} dx = \int \varphi^* f_{\lambda=2} dx = \alpha$
\compact
\subsection{UMP}
...
\compact
\subsection{Simultaneous Interval}
$C_t(X), t \in \mathcal{T}$ are $1-\alpha$ simultaneous confidence intervals for $\theta_t, t\in \mathcal{T} \Leftrightarrow$
$\inf_{P\in \mathcal{P}} P(\theta_t \in C_t(X) \text{ for all } t \in \mathcal{T}) \geq 1 - \alpha$
asymptotic CI if $\lim_{n\rightarrow \infty} P(\theta_t \in C_t(X) \text{ for all } t \in \mathcal{T}) \geq 1 - \alpha$
...
(Multivariate Normal) $||(X-\mu)/\sigma||^2 < \chi_{p}^2$
\compact
\subsection{UMPU}
Exists for one-param,
\compact
...
\compact
\subsection{Pratt's theorem}
Suppose $vol(C(x)) = \int_C(x) d\theta'$ is finite, then expected volume of $C(X)$
$E[vol(C(x))] = \int_{\theta'\neq\theta} P(\theta' \in C(x)) d\theta'$
\compact
\subsection{Uniformly most accurate (UMA)}
$\theta \in \Theta$ and $\Theta' \subset \Theta$ that does not contain true $\theta$,
$C(X)$ is $\Theta'-$UMA $\Leftrightarrow$ $P(\theta' \in C(X)) \leq P(\theta' \in C_1(X))$ for any other $C_1(X)$
$C(X)$ is UMA $\Leftrightarrow$ it is $\Theta'-$UMA with $\Theta' = \{\theta\}^c$
$\Rightarrow$ inverting $H_0: \theta = \theta_0$ and $H_1: \theta \neq \theta_0$
\compact
...
require unimodal wrt $x$ at $x_0$: $f'(x_0) = 0$ $f'(x)<0, x < x_0$ and $f'(X)>0, x > x_0$
\compact
...
$\sup_{\theta\in\Theta}R_S(\theta) \geq \int_{\Theta_\pi}R_S(\theta)d\pi \geq \int_{\Theta_\pi}R_T(\theta)d\pi = \sup_{\theta\in\Theta}R_T(\theta)$
\compact
\subsection{Bayesian admissibility}
...
\textit{Not true for improper priors unless it is limit of Bayes rule}, Improper priors require excessive risk
ignorable, take limit and observe if risk is admissible.
\compact
\subsection{Simultaneous credible set}
...
$T_n = \sqrt{n} (\tilde{\theta_n}-\hat{\theta_n}) \sim \mathcal{N}(0, V^*)$
asymptotically.
(Well-specified)
...
(differ from bayesian CR, $\theta_*$ is the projection of $P_*$ to parameter
space)
\compact
...
\compact
\subsection{LM MLE}
$X_i = \beta_0 + \beta_1 t_i + \epsilon_i$,
$D:=\sum (t_i - \bar{t})^2$,
$\hat\beta_1 = \frac{1}{D}(\sum t_i(X_i-\bar{X}))$
and $\hat\beta_0 = \bar{X} - \hat\beta_1 \bar{t}$
and $\hat\sigma^2 = \frac{1}{n}\sum(X_i-\bar{X})^2 - \frac{D}{n}\hat\beta_1^2$ (?)
\compact
\subsection{LM solution property}
\circled{a} $E(\hat\beta) = \beta$,
\circled{b} $Var((Z^T Z)^{-1} ZX ) = (Z^T Z)^{-1}Z Var(X) Z^T (Z^TZ)^{-1}$
\circled{c} $Var(\ell^T\hat\beta) = \sigma^2 \ell^T(Z^TZ)^-\ell$
\circled{d} $Cov(\hat\beta)=\sigma^2 \begin{pmatrix}
        n & \sum t_i \\ \sum t_i & \sum t_i^2
    \end{pmatrix}^{-1} = \frac{\sigma^2}{nD} \begin{pmatrix}
        \sum t_i^2 & -\sum t_i \\ -\sum t_i & n
    \end{pmatrix}$
\compact
\subsection{LM tricks}
$X-Z\hat\beta = P_{Z\perp} X, Z\hat\beta = P_Z X$.
$\exists W \in \mathcal{R}^{n\times(n-r)}$ s.t. $W^T W = I_{n-r}$ and $WW^T = P_{Z\perp} = I_{n}$
\compact
...
\compact
\subsection{CI for $H_0: \theta = \theta_0$, $H_1: \theta < \theta_0$}
$A(\theta_0) = \left\{X: \ell^T \hat\beta - \theta_0 > -t_{n-r, \alpha}\sqrt{\ell^T(Z^TZ)^-\ell SSR/(n-r)}\right\}$
\compact
\subsection{CI For $H_0: \theta = \theta_0$, $H_1: \theta\neq \theta_0$}
$A(\theta_0) = \left\{X: |\ell^T\hat\beta - \theta_0| < t_{n-r, \alpha/2}\sqrt{\ell^T(Z^TZ)^-\ell SSR/(n-r)}\right\}$
\compact
\subsection{Asymptotic CI}
Does not require normality of noise
$C(X) = \left\{\beta: (\hat\beta - \beta)^T (Z^TZ)(\hat\beta-\beta) \leq \chi_{p, \alpha}^2 SSR/(n-p)\right\}$
$SSR = ||X-Z\hat\beta||^2$
\compact
\subsection{Linear Estimator}
Linear estimator for linear model $X=Z\beta + \epsilon$ is linear function of $X$.
e.g. $\ell^T\hat\beta = \ell^T(Z^TZ)^-Z^TX = C^T X$, $Var(c^TX) = c^T Var(X) c = c^T Var(\epsilon) c$
\compact
\subsection{Bivariate Normal density}
$X_i$ are iid from bivariate normal
$f(X) = \frac{1}{(2\pi\sigma_1\sigma_2\sqrt{1-\rho^2})^n}\exp\left\{ - \frac{||Y_1-\mu_1 1_n||^2}{2\sigma_1^2(1-\rho^2)} + \frac{\rho(Y_1-\mu_11_n)^T(Y_2-\mu_21_n)}{\sigma_1\sigma_2(1-\rho^2)} - \frac{||Y_2 - \mu_21_n||^2}{2\sigma_2^2(1-\rho^2)} \right\}$,
Testing $H_0: \rho=0$, $H_1: \rho \neq 0$ with
$\theta = \frac{\rho}{\sigma_1\sigma_2(1-\rho^2)}$,
$Y=\sum_{i=1}^n X_{i1}X_{i2}$,
$U = \left(\sum_{i=1}^n X_{i1}^2, \sum_{i=1}^n X_{i2}^2, \sum_{i=1}^n X_{i1}, \sum_{i=1}^n X_{i2}\right)$,
Sample correlation coefficient:
$R=\frac{\sum_{i=1}^n(X_{i1}-\bar{X}_1)(X_{X_{i2} - \bar{X}_2})}{\left\{\sum_{i=1}^n(X_{i1}-\bar{X}_1)^2\sum_{i=1}^n(X_{i2}-\bar{X}_2)^2\right\}^(1/2)}$,
$T=\sqrt{n-2}R/\sqrt{1-R^2}\sim t_{n-2}$ under $H_0: \rho=0$,
UMPU test reject $|T| > t_{n-2, \alpha/2}$
\compact
\subsection{Conditional normal}
$x\sim N_x(\mu, \Sigma)$, $x=[x_a, x_b]^T$, $\mu=[\mu_a, \mu_b]^T$,
$\Sigma=[[\Sigma_a, \Sigma_c], [\Sigma_c^T, \Sigma_b]]^T$,
$p(x_a|x_b)=N_{x_a}(\hat\mu_a, \hat\Sigma_a)$,
$\hat\mu_a = \mu_a + \Sigma_c\Sigma_b^{-1}(x_b-\mu_b)$,
$\hat\Sigma_a = \Sigma_a - \Sigma_c\Sigma_b^{-1}\Sigma_c^T$
\end{document}